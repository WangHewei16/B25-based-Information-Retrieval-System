...
def read_documents():
    ...
    for folder in files:
        next_files = os.listdir('./documents/' + folder)
        for file in next_files:
            N += 1
            position = './documents/' + folder + '/' + file
            document_len = 0
...
def find_retrieved_documents() -> dict:
    retrieved_dict = dict()
    with open('./files/output.txt', 'r') as lines:
        for line in lines:
            line = line.replace('\n', '')
            ...
            if query_id not in retrieved_dict:
                ...
                retrieved_list.append([document_word, rank])
            else:
                retrieved_list = retrieved_dict[query_id]
                retrieved_list.append([document_word, rank])
...
# This function is to store relevant documents in the dictionary.
...
def build_term_index() -> dict:
    ...
    term_index = dict()
    file = open('./files/terms_large.txt', 'r')
...
def execute_query(term_index: dict):
    ...
    while entered_query != 'QUIT':
        ...
        # Design decision: only return Top15
        rank = 0
...
# The function is special in dealing with large corpus because there are several unjudged documents in large corpus.
# My system returns a dict which includes ID of judged documents.
def execute_relevance_judgment() -> dict:
    relevance_judgment_dict = dict()
    with open('./files/qrels.txt', 'r') as lines:
        for line in lines:
            line = line.replace('\n', '')
            line = line.strip()
            ...
            query_id = words[0]
            document = words[2]
            if query_id not in relevance_judgment_dict:
                relevance_judgment_dict[query_id] = set()
                document_dictionary = relevance_judgment_dict[query_id]
                document_dictionary.add(document)
            else:
                document_dictionary = relevance_judgment_dict[query_id]
                document_dictionary.add(document)
    return relevance_judgment_dict


# This function stores each terms' documents BM25 score in a generated file named terms_large.txt
# Format: term  document_id  score
# It is an efficient way to generate BM25 index because the system only needs to calculate BM25 score in each document for each terms appearing in the query.
# So the system stores terms as the key and puts a dictionary storing different documents' score as value.
def generate_terms_file():
    ...
    if not os.path.isfile('./files/terms_large.txt'):
        ...
        for term in terms:
            ...
            for document_id, frequency in terms[term][1:]:
                ...
                score = (frequency * 2) / (frequency + 0.25 + (0.75 * document_cur_length / average_length)) * math.log2((N - n_i + 0.5) / (n_i + 0.5))
                if score <= 0.0:
                    score = 0.0
        ...
        # Generate then write terms_large.txt file to store the score of term in each document
        file = open('./files/terms_large.txt', 'w')
        for term in terms:
            file.write(term + '\n')
...
def execute_evaluation():
    ...
    relevance_judgment_list = execute_relevance_judgment()
    for query_id in found_retrieve:
        relevant_documents = found_relevant[query_id]
        relevant_documents_number = len(relevant_documents)
        N = 0
        ...
        for document in found_retrieve[query_id]:
            document_id = document[0]
            # Taking advantage of relevance judgment function to generate a list to judge the condition of relevance.
            # If not in this list, it means the document is not judged, so could ignore it to enhance efficiency.
            if document_id not in relevance_judgment_list[query_id]:
                continue
            else:
                if document_id in relevant_documents:
...
if __name__ == '__main__':
    ...
    if parser_parse_args.m == "manual":
        ...
        print('Query manually command:  [python search_large.py -m manual]')
        print('Execute evaluation command:  [python search_large.py -m evaluation]')
